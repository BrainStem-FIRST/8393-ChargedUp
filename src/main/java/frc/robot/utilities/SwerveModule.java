...
public class SwerveModule {
    ...
    public void setDesiredState(SwerveModuleState desiredState, boolean isOpenLoop){
        ...
        double angle = (Math.abs(desiredState.speedMetersPerSecond) <= (DrivetrainConstants.maxSpeed * 0.01)) ? lastAngle : desiredState.angle.getDegrees(); //Prevent rotating module if speed is less then 1%. Prevents Jittering.
        steerMotor.set(ControlMode.Position, Conversions.degreesToFalcon(angle, DrivetrainConstants.angleGearRatio)); 
        lastAngle = angle;
    }

    private void resetToAbsolute(){
        double absolutePosition = Conversions.degreesToFalcon(getCanCoder().getDegrees() - absoluteEncoderOffset, DrivetrainConstants.angleGearRatio);
    ...
    public SwerveModuleState getState(){
        double velocity = Conversions.falconToMPS(driveMotor.getSelectedSensorVelocity(), DrivetrainConstants.wheelCircumference, DrivetrainConstants.driveGearRatio);
        Rotation2d angle = Rotation2d.fromDegrees(Conversions.falconToDegrees(steerMotor.getSelectedSensorPosition(), DrivetrainConstants.angleGearRatio));
