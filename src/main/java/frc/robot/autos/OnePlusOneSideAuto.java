...
public class OnePlusOneSideAuto extends SequentialCommandGroup {

        public static final class AutoConstants { // TODO: The below constants are used in the example auto, and must be
                                                  // tuned to specific robot
                public static final double k_maxSpeedMetersPerSecond = 1; // 1.8
                public static final double k_maxAccelerationMetersPerSecondSquared = 0.9; // 0.9
                public static final double k_maxAngularSpeedRadiansPerSecond = Math.PI / 10; // pi/10
                public static final double k_maxAngularSpeedRadiansPerSecondSquared = Math.PI / 10; // pi / 10
        ...
        public OnePlusOneSideAuto(Swerve s_Swerve, Lift m_lift, Collector m_collector, Extension m_extension) {
                ...
                Trajectory collectCargoTrajectory = TrajectoryGenerator.generateTrajectory(
                                ...
                                // facing forward
                                new Pose2d(1, 0, new Rotation2d(Math.toRadians(180))),
                                config);

                config.setReversed(true);
                // An example trajectory to follow. All units in meters.
                Trajectory depositCargoCTrajectory = TrajectoryGenerator.generateTrajectory(
                                // Set the origin at (5,0) facing the +X direction
                                // Robot starts facing the poles
                                new Pose2d(5.5, 0, new Rotation2d(Math.toRadians(0))),
                                // Pass through these two interior waypoints, making an 's' curve path
                                ...
                                // End 5 meters behind ahead of where we started, rotating 180 degrees, now
                                // facing forward
                                new Pose2d(3, 0, new Rotation2d(Math.toRadians(180))), //3.68 for blue //3.66 for red
                ...
                SwerveControllerCommand collectCargoCommand = new SwerveControllerCommand(
                    collectCargoTrajectory,
                                s_Swerve::getPose,
                ...
                SwerveControllerCommand depositCargoCommand = new SwerveControllerCommand(
                    depositCargoCTrajectory,
                                s_Swerve::getPose,
                                SwerveConstants.k_swerveKinematics,
                                new PIDController(AutoConstants.k_pXController, 0, 0),
                                new PIDController(AutoConstants.k_pYController, 0, 0),
                ...
                addCommands(
                                ...
                                //                 .resetOdometry(collectCargoTrajectory.getInitialPose())),
                                // m_highPoleApproach,
                                // m_depositSequenceCommandGroup,
                                // m_intakeOff, 
                                // collectCargoCommand,

                                new InstantCommand(() -> allignToCargo(s_Swerve))
        ...
        private void allignToCargo(Swerve s_Swerve ) {

            PIDController m_xPID = new PIDController(BalanceConstants.k_P, BalanceConstants.k_I,BalanceConstants.k_D);
            PIDController m_yPID = new PIDController(BalanceConstants.k_P, BalanceConstants.k_I,BalanceConstants.k_D);

            m_xPID.setTolerance(2);
            m_yPID.setTolerance(2);

            double blockXPose = -10;
            double blcokYPose = -15;

            while (true) {
            double xOutput = MathUtil.clamp(m_xPID.calculate(blockXPose), -0.15, 0.15);
            double yOutput = MathUtil.clamp(m_yPID.calculate(blcokYPose), -0.15, 0.15);
